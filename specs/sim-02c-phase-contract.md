...
2026-07-14の独立監査後V2実M-B runは5 route、raw payload 2,025 files / 404,113,376 bytes、derived 23を読み取り、network I/Oなしで同じcompilation hashを再生成した。V1の`complete 1 / partial 4`は証拠snapshotを取得再現と誤って同一視したためsupersededとする。V2 acquisition integrityは`reproduced 0 / snapshot_bound 0 / partial 5`、production policy resolved 0/5。固定235件はcandidate 219 / conflict 16 / verified 0で、promotion unresolvedは235。Catalog V2はspecies 235、moves 490、abilities 180、items 117、types 18、Mega relation candidate 70、required field 8,024、verified 0、runtime lowerable 0。assessmentは`declared_workbench_surfaces_and_known_gap_hints`内でacquisition 2,104、policy 35、mapping 705、Catalog 8,024、その他6の合計10,874 blockerを全件列挙し、未宣言依存の完全性はfalseとする。23 derivedのうち15件はcross-route/derived parent、未登録transform、中間物またはruntime依存を現契約で表現できず、8件はlineage requirement自体が未宣言である。
