...
- AI-01 readiness promotion: **FAIL-CLOSED SCAFFOLD / POSITIVE PATH NOT IMPLEMENTED** — 現`source-to-capability-bundle-v1`はunverified intakeと空development corpusを固定し、`catalog_emit_eligible=false`を強制するためsealを発行できない
...
| `inputs_available` | SIM-01全基盤、source-bound M-B/M-A RegulationSnapshot、M-B 235フォームTargetPoolSnapshot、recomputed coverage/diffとsynthetic-NO-GO-only rehearsal v1、strict content-addressed CaptureStore、manifest-bound GroundingTrace、allowlist/evidence-bound AI Env、内部ではversion/hash/seed/RNGへ結合しpolicy-facing resultには公開identityだけを返すpolicy-free `reset`/`step` adapter、source-bound Catalog intake、explicit mapping/fixed-point closure/6次元/probe/grounding/holdoutを持つTargetCapability pipeline、source-lockからmapping evidence・intake-only Catalog candidate・semantic/execution registry・probe・MechanicCoverageMatrix・理由付きNO-GO reportまでをcontent-addressed生成するv1 compiler、v1 substanceを再検証して自己申告VERIFIEDを拒否するfail-closed readiness resolver、standard-stat-formula検証付きReplay互換generic mega、全階層strict Replay decoder、6→3 sealed team preview、partial-observation selection/battle baseline、paired-seat arena、prebattle非内包を明示したbattle-Replay archive/evidence manifest、frozen synthetic report。 |
...
AI-01後の大きな目的は`SIM-02B Production Catalog Promotion + Evidence-backed Scenario Corpus`とする。intake診断専用v1を凍結し、compilerが列挙した235 mapping、788 semantic selector、118 capability gapをauthoritative sourceまたはactual traceで昇格するresolver-backed v2を別型で作る。その同じidentityから非空development corpusと系譜分離したexternal-holdout corpusを生成し、技priority/effect、特性、道具、base stats、Mega relationには構造化evidenceとengine-backed positive probeを同時に要求する。promotion/catalog/scenario/partition/holdout hashへ結合したreadiness sealが発行されるまでMCTS/RL/LLMへChampions candidateを渡さない。旧PJ由来データのlicenseが確認できない限り、元データ・Catalog・capture・派生データを再配布しない。詳細は`specs/sim-02b-phase-contract.md`を正本とする。
