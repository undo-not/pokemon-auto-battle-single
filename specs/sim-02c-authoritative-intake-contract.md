...
| `purpose` | 現行M-Bの235 target memberと技・特性・道具・Mega relationについて、権利・出典・namespace・form・field-level provenanceの不足をproduction昇格と分離して機械可読化し、新レギュレーションで再生成できるreview workbenchを作る。 |
| `objective_variable` | `acquisition_route_integrity_rate`、`source_policy_resolution_rate`、`authoritative_mapping_rate`、`catalog_required_field_evidence_rate`、`assessment_blocker_enumeration_rate`を測る。最初の4値はpromotion条件ではすべて`1.0`、最後は`declared_workbench_surfaces_and_known_gap_hints`という明示scope内の固定分母上の不足を漏れなく列挙して`1.0`とする。現実入力では不足を分母から除外しない。未宣言の外部依存まで完全列挙したとは主張せず、`undeclared_dependency_enumeration_complete: false`を固定する。 |
...
| `output_models` | `SourceAcquisitionReviewV2`、`AuthoritativeMappingWorkbenchV2`、`AuthoritativeCatalogV2Workbench`のtransport contract V2、`AuthoritativeIntakeAssessmentV2`、content-addressed `AuthoritativeIntakeCompilationV2`。V1 Schemaは旧run再検証用に凍結保存し、V2 loaderはV1 planを拒否する。全出力は常に`authorization_status: not_authorization`で、既存promotion V2/V3 inputへ暗黙変換しない。 |
...
| `provisional_coefficients` | coverage、mapping、permission、promotionのthresholdは置かない。planの`expected_min_files`と`expected_min_records`は`PD-013`のroute-local anomaly floorであり、完全性・正確性・利用許諾を示さず、235分母またはrequired field分母を縮小しない。 |
...
assessmentはsource、policy、mapping、Catalog field、effect/handler、scenario、grounding、holdout、trustの不足を、最初の失敗で停止せずソート済み全件として返す。blockerは`stage`、`code`、`subject`、`evidence_required`、`restart_condition`を必須とする。完全列挙の主張は宣言済みworkbench surfaceと既知のgap hintだけに限定し、未宣言依存の完全性とは分離する。同じ入力bytesとplanからbyte-identical report hashを再生成する。

V2が証明できる取得完全性の上限は`snapshot_bound`である。取得処理を同一入力条件で再実行した因果証拠の型を持たないため、routeの`reproduced`と`acquisition_route_integrity_rate_ppm > 0`をSchemaで拒否する。取得再現の主張は、実行証跡契約を追加した将来versionでのみ導入する。
...
3. 旧PJ raw/processedをコピーせずroute単位でhash集約する。直接表現可能なparser/derived lineageだけをsnapshot-boundとし、cross-route、derived-parent、未登録transform/intermediate/runtime依存は構造化gapとして固定する。
...
- evidence、raw payload、derived artifactの宣言pathとopen-handle file keyを全route共通registryで一意にし、hardlink、同一file、異なる役割への同一bytes流用、rawをderived出力に再利用する循環を拒否する。
...
- raw manifestの各`payload.saved_to`は宣言済みraw inventoryのpath/suffix内になければならず、canonical path重複を拒否する。manifestとinventoryはexactly-oneで相互結合し、両走査のpath/byte/hash projectionが同じsnapshotでなければcompile自体を中断する。存在・byte count・SHA-256が一致して初めてacquisition chainを満たす。
...
- `lineage_requirements`と`lineage_gap_hint`は排他的とする。後者は親route/artifact、未登録path、runtime dependencyと理由codeを持ち、`lineage_status: unrepresentable`かつ`derived_lineage_graph_unrepresentable` blockerにしかならない。gap hint、source label、record count、LLM推論で`snapshot_bound`へ昇格しない。
